...
- Streaming proxy support — `stream: true` requests now proxy as real Server-Sent Events, rescanned incrementally after every chunk, with a mid-stream cutoff (`finish_reason: "content_filter"`) if content turns malicious partway through. Verified live: the specific violating chunk is suppressed entirely, not just "some tokens after." One precisely-stated limitation: a detectable pattern split exactly across a chunk boundary can partially leak before the second half completes it.
- Dashboard — `GET /dashboard`, a single static page polling `GET /api/v1/audit/recent`. Screenshotted against live seeded data before shipping (not just reviewed as source), which caught and fixed two real issues: an illegible risk indicator, and tool-call/MCP entries with no indication of which tool was involved (fixed by adding an optional `detail` field to the audit log — a controlled identifier, not arbitrary text).
