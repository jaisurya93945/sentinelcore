...
| 5 | **Authentication** | **DONE** | `sentinelcore/core/auth.py`, off by default, tested end-to-end incl. live demo |
...
| 35 | Semantic/ML detection | **PARTIAL — real, benchmarked, shipped off-by-default** | Learned lexical classifier (TF-IDF + calibrated logistic regression), `sentinelcore/detectors/ml_classifier/`. Head-to-head on an identical held-out split (`scripts/compare_baselines.py`): recall 27.54% -> 85.51% (3.10x), precision 100.00% -> 93.65%, FPR 0.00% -> 5.00%. Not a transformer and not claimed to be. Its measured cost drove a policy change (Finding 5): the provenance-escalation rules it would have shipped with were removed after the ablation showed them net-negative |
