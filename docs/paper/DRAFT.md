# Which Security Signals Actually Earn Their Place? An Empirical Study of a Layered Control Plane for LLM Agents

*(Working title. The contribution is an evaluation of which controls in a layered AI-security architecture measurably help, not a claim that our detector is best. §5.1 replicates a known result about lightweight detectors; the novel material is §5.3–§5.5.)*
...
## 0. Status of every claim in this paper

Each capability is labelled against the evidence audit (`docs/evidence/EVIDENCE_TABLE.md`, regenerated by `scripts/audit_evidence.py` from result files only). Nothing below is asserted from prose or memory.

| Capability | Status | Evidence |
|---|---|---|
| Prompt-injection, obfuscation, PII, secret detection | **TESTED** | unit tests; measured on a held-out split |
| Learned detector outperforms rules | **EXPERIMENTALLY VALIDATED** | 10 seeds, disjoint CIs, McNemar p < 5.6×10⁻⁶ |
| Tool-call inspection and deterministic tool authorization | **TESTED** | regression tests incl. a reproduced bypass |
| Risk scoring, policy engine, enforcement | **TESTED** | unit tests; enforcement status distinct from decision |
| Sanitization with mandatory re-scan | **TESTED** | escalates when cleaning reveals an attack |
| Human approval workflow | **TESTED** | fail-closed on expiry; separation of duty |
| RAG/context scanning, provenance tagging | **TESTED** | origin propagated end to end |
| Detector choice dominates policy choice | **EXPERIMENTALLY VALIDATED** | +50.6pp APR, disjoint bootstrap CIs |
| Provenance helps under an oracle detector | **EXPERIMENTALLY VALIDATED (simulation)** | +47.1pp, disjoint CIs — but the oracle has zero false positives by construction |
| Provenance helps under a learned detector | **QUALIFIED** | +11.8pp, CIs overlap at the edges; operating-point dependent |
| Provenance under a semantic detector | **HYPOTHESIZED** | **not reproducible from any artifact in this repository** — see §5.5 |
| LLM confidence is unusable for graded policy | **EXPERIMENTALLY VALIDATED (n=149) / QUALIFIED (logprob arm, n=51)** | 8 distinct values, 1 in the ambiguous band, 100% confident errors |
| Rate limiting, payload caps | **IMPLEMENTED** | off by default; per-process, not distributed |
| MCP tool-definition scanning | **TESTED** | static discovery only; no runtime relay |
| Over-defense behaviour | **PLANNED** | corpus has 5/399 hard negatives; external benchmark unreachable from the dev environment |
| Comparison against an industry guardrail | **PLANNED** | harness built; blocked on model download |
| Stateful/session security, memory security, action graph | **PLANNED** | not implemented |
...
This paper is not an argument that SentinelCore detects better than existing tools. §5.1 replicates a published result and says so. The question we can answer, and which the layered-defence literature largely does not, is **which layers earn their place once detector quality is controlled for**.
...
This is the proposed mechanism behind §5.3's provenance result: the policy layer is not underperforming on the semantic detector so much as being starved of the graded signal it acts on.

**The agent-level number that would quantify this is currently withheld** — see §5.5. The confidence-distribution measurement above (n=149) is reproducible from an artifact; the corresponding ablation arm is not.
...
### 5.5 A withheld result, and why

An earlier version of this paper reported provenance under the semantic detector at **+1.2pp APR**, contrasted against +11.8pp for the learned detector. That number is **withheld from this draft.**

The evidence audit (`scripts/audit_evidence.py`) found that the ablation artifact checked into the repository has `L_semantic_flat` **exactly equal** to `C_tool_authz` and `M_semantic_prov` exactly equal to `E_prov_rules`. The cause is a property of the system working correctly: the semantic detector **fails open** when its response cache is absent, which is the right behaviour for a gateway that must not go down when a third-party API is unreachable — and precisely the wrong behaviour for an experiment, because the configuration silently degenerates into its own control while still producing a well-formed artifact with plausible numbers.

The +1.2pp figure was measured on a machine that had the cache. It is probably correct. It is nonetheless **not reproducible from any artifact in this repository**, and the standing rule is that repository artifacts are the source of truth over prose or recollection. The audit now detects this collapse by equality-checking the semantic arm against its control, and marks the row unpublishable until regenerated.

We report this rather than quietly restating the number because the failure mode generalises: **a fail-open component inside an ablation converts a missing dependency into a silently degenerate condition**, and neither the artifact nor the summary table shows any sign of it. Any evaluation harness containing an optional component that degrades gracefully has this exposure.
